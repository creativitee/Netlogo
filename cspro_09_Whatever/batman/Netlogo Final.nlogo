...
to setup
  ...
  create-fighters num_fighters [fighters-setup]
  create-soldiers num_soldiers [soldiers-setup]
  import-pcolors "Basic2.png"
end

;The Movement and what actually happens
to W
...
to Easy-Difficulty
  ...
  if breed = fighters [set HP random 100]
...
;Fighting
to attack
  if breed = players [if any? fighters-here [set HP HP - Damage]]
end
...
to Setup-GUN
  ...
  set old-gun-heading [heading] of gun 0
  set was-mouse-down? false
end


to Fire
  let x-start ([size] of gun 0) * sin ([heading] of gun 0) / 2
  let y-start ([size] of gun 0) * cos ([heading] of gun 0) / 2
  create-tears 1 [
    setxy x-start y-start
    set shape "Tears"
    set heading [heading] of gun 0
    set color red
    set size 1.5
  ]
  tick
...
To Go
  ...
  if gun-heading != old-gun-heading [
    ask gun 0 [set heading gun-heading]
    ;set [heading] of gun 0 gun-heading
    set old-gun-heading gun-heading
  ]
  ...
  if Mouse-control? [
    ...
    if mouse-down? and not was-mouse-down? [ fire ]
