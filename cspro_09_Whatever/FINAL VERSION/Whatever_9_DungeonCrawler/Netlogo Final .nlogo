...
breed [Tears-UPs Tears-UP]
breed [Tears-Damages Tears-Damage]
breed [Health-UPs Health-UP]
players-own [Tear-Speed Tear-Damage HP]
spiders-own [HP Tear-Damage]
worms-own [HP Tear-Damage]
bosses-own [HP Tear-Damage]
...
;Turtles Setup
to spiders-setup
  ask spiders [set shape "spiders"
    set HP 3
    set size 4
    set Tear-Damage 1
    setxy random 40 27]
  ask spiders [set heading towards player 0]
end
...
to spiders-movement
  if breed = spiders [
    ...
    if any? players-here [ask players [set HP HP - 0.005]]]
end

to worms-setup
  ask worms [set shape "worm"
    set HP 4
    set size 5
    set Tear-Damage 1
    setxy random 40 27]
...
to boss-movement
    ...
    fd 0.008
    set heading towards player 0
    if any? players-here [ask players [set HP HP - 0.01]]]
end
...
to hp-labeling
  ifelse showHP? 
  [set label precision HP 2]
  [set label ""]
end

;Fighting
to attack
  if breed = spiders or breed = worms or breed = bosses [if any? bullets-here [set HP HP - 0.05
      set pcolor 16]]
end
...
to Next-Map
  if count spiders <= 0 
  [ask patches [if pcolor = 1.3 [set pcolor white]]]
  if count worms <= 0 
  [ask patches [if pcolor = 1.3 [set pcolor white]]]
  if count spiders > 0 [ask patches [if pcolor = white [set pcolor 1.3]]]
  if count worms > 0 [ask patches [if pcolor = white  [set pcolor 1.3]]]
  if count players > 0
  [if [pcolor] of player 0 = 25
  [Secret-Room]]
  
  ask players [if any? hearts-here [set HP HP + 1]]
  if count players > 0
  [if [pcolor] of player 0 = 15 
  [Boss-Room]]
  if count players > 0
  [if [pcolor] of player 0 = white [random-map
    ask players [setxy 0 0] random-mobs]]
end

to random-map
  let x random 3
  if x = 0
  [import-pcolors "Final_editV2.png"]
  if x = 1
...
to random-boss
  ...
  [set shape "Mulligan"]
  if y = 2
  [set shape "ninja isaac"]
end

to random-mobs
  let n random 2
  if n = 0
  [create-spiders 5[spiders-setup]]
  if n = 1
  [create-worms 5[worms-setup]]
end

to random-powerups
  let z random 3
  if z = 0
  [create-Health-UPS 1 [set shape "Hearts-UP" setxy -10 0 set size 4]]
  if z = 1
  [create-Tears-UPs 1 [set shape "Tears-UP" setxy 0 -5 set size 4]]
  if z = 2
  [create-Tears-Damages 1 [set shape "Tears-Damage" setxy 10 0 set size 4]]
end


to Secret-Room
  import-pcolors "Secret_room_inside.png"
  ask players [setxy 0 0]
  repeat random 3 [random-powerups]
end

to Boss-Room
  import-pcolors "Boss_room_from_left.png" 
  ask players [setxy 0 0]
  create-bosses 1[bosses-setup]
end
...
To Go
  ask players [if any? Tears-UPs-here [set Tear-speed Tear-speed + 0.005 ask Tears-UPs [die]]]
  ask players [if any? Tears-Damages-here [set Tear-Damage Tear-Damage + 1 ask Tears-UPs [die]]]
  ask players [if any? Health-UPs-here [set HP HP + 1 
      ask Health-UPs [die]]]
  if [HP] of player 0 <= 0 [user-message "Game Over. Press Halt and Setup to Play Another!"]
  if count bosses > 0
  [if [HP] of one-of bosses <= 0 [user-message "Thanks for playing!" user-message
        "If you want to play another Level, Press Ok, if you do not wish to, Press Halt." import-pcolors "Final_edit.png"]]
  ask players [if HP <= 0 [die]]
  ask bosses [if HP <= 0 [die]]
  ask spiders [if HP <= 0 [die]]
  ask worms [if HP <= 0 [die]]
  Next-Map
  ask bosses[boss-movement]
  ask spiders[spiders-movement]
  ask worms [worms-movement]
  ask turtles [attack]
  if count bullets > 0 [
    ask bullets [ 
      fd 0.03
...
to Labeling
  ...
  ask worms [hp-labeling]
end

;HEALTH, DAMAGE, AND DAMAGE SPEED REPORTERS

to-report health
  report [precision HP 2] of players
end

to-report damage
  report [Tear-Damage] of players
end

to-report damage-speed
  report [Tear-Speed] of players
end
@#$#@#$#@
GRAPHICS-WINDOW
210
...
If you aren't familiar with Binding of Isaac or a Dungeon Crawler, what it is is a RPG(Role Playing Game) where you are the main character and explore and try to defeat enemies in a dungeon like space. After defeating some enemies, you can fight the boss and win the game. In Binding of Isaac, there are powerups and specific projectiles that you, the main character, use to fight with enemies. These projectiles, called tears, can be increased in damage and attack speed via powerups. These, powerups, are located in a secret room that may or may not have powerups. In this version, powerups are Increased Tear Damage, Health, 
...
Mr Brooks for the projectiles and patch painter code
Binding of Isaac for model references and turtle shapes
@#$#@#$#@
default
true
0
...
false
0
Rectangle -2674135 true false 45 105 240 150
Rectangle -2674135 true false 75 75 120 90
Rectangle -2674135 true false 150 90 225 105
Rectangle -2674135 true false 90 90 135 90
Rectangle -2674135 true false 60 150 225 165
Rectangle -2674135 true false 75 165 210 180
Rectangle -2674135 true false 90 180 195 195
Rectangle -2674135 true false 105 195 180 210
Rectangle -2674135 true false 120 210 165 225
Rectangle -2674135 true false 135 225 150 240
Rectangle -2674135 true false 60 90 135 105
Rectangle -2674135 true false 165 75 210 90

hearts-up
false
0
Rectangle -2674135 true false 15 105 210 150
Rectangle -2674135 true false 45 75 90 90
Rectangle -2674135 true false 120 90 195 105
Rectangle -2674135 true false 90 90 135 90
Rectangle -2674135 true false 30 150 195 165
Rectangle -2674135 true false 45 165 180 180
Rectangle -2674135 true false 60 180 165 195
Rectangle -2674135 true false 75 195 150 210
Rectangle -2674135 true false 90 210 135 225
Rectangle -2674135 true false 105 225 120 240
Rectangle -2674135 true false 30 90 105 105
Rectangle -2674135 true false 135 75 180 90
Rectangle -1184463 true false 255 75 270 240
Rectangle -1184463 true false 240 90 285 105
Rectangle -1184463 true false 225 105 240 120
Rectangle -1184463 true false 240 105 300 120
...
false
0
Rectangle -16777216 true false 90 180 180 210
...
Rectangle -11221820 true false 195 135 225 165
Rectangle -11221820 true false 60 120 90 150
Rectangle -11221820 true false 75 105 90 120
Rectangle -11221820 true false 90 120 120 135
Rectangle -11221820 true false 180 135 210 150
Rectangle -1 true false 150 210 165 225
Rectangle -16777216 true false 150 90 195 90
Rectangle -16777216 true false 240 90 240 105
Rectangle -16777216 true false 180 15 255 15
...
Rectangle -16777216 true false 105 240 135 255
Rectangle -16777216 true false 135 255 165 270
Rectangle -16777216 true false 165 255 225 270
Rectangle -16777216 true false 75 45 105 60
Rectangle -16777216 true false 105 60 135 75
Rectangle -16777216 true false 135 45 165 60
Rectangle -16777216 true false 165 30 240 45
Rectangle -16777216 true false 225 45 255 60
...
Rectangle -16777216 true false 240 120 270 135
Rectangle -16777216 true false 255 135 270 165
Rectangle -16777216 true false 255 165 285 180
...
Rectangle -16777216 true false 15 135 30 150
Rectangle -16777216 true false 30 150 45 165
Rectangle -16777216 true false 15 165 30 180
Rectangle -16777216 true false 30 180 45 210
Rectangle -16777216 true false 15 210 30 225
Rectangle -16777216 true false 225 240 240 255
Rectangle -16777216 true false 240 210 255 240
Rectangle -2064490 true false 120 120 180 165
Rectangle -2064490 true false 165 45 225 105
Rectangle -2064490 true false 180 165 240 240
Rectangle -2064490 true false 75 210 105 255
